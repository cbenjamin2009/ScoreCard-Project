...
const findHeaderIndex = (rows) => {
  ...
  rows.forEach((row, index) => {
    ...
    const metricIndex = labels.findIndex((cell) => cell === 'metric');
...
const buildColumnWindow = (headerRow, cadenceConfig, headerIndexes) => {
...
const extractMetricRows = (rows, cadence) => {
  ...
  const metricIndex = headerLabels.findIndex((cell) => cell === 'metric');
  if (metricIndex === -1) {
    throw new Error('Unable to locate the "Metric" header row inside the worksheet.');
  }
  ...
  const { valueStart, valueEnd } = buildColumnWindow(headerRow, cadenceConfig, {
    metricIndex,
    descriptionIndex,
    panicIndex,
  });
...
const detectAvailableCadences = (sheetNames) => ({
...
export const getScorecardData = ({
  ...
  const availableCadences = detectAvailableCadences(sheetNames);
  let cadenceKey = requestedKey;
  let cadenceFallback = false;

  if (!configuredSheet) {
    if (requestedKey === 'weekly' && !availableCadences.weekly && availableCadences.monthly) {
      cadenceKey = 'monthly';
      cadenceFallback = true;
    } else if (requestedKey === 'monthly' && !availableCadences.monthly && availableCadences.weekly) {
      cadenceKey = 'weekly';
      cadenceFallback = true;
    }
  }
  ...
  let sheetName = resolveSheetName({
    cadenceKey,
    sheetNames,
    configuredSheet,
  });
  let workbookMeta = { path: workbookPath, sheet: sheetName, mtimeMs: workbookStats.mtimeMs };
  let cacheContext = getCacheContext(workbookPath, sheetName);
  ...
  let { metrics, periodLabels } = extractMetricsForSheet(worksheet, cadenceKey) || {
    metrics: [],
    periodLabels: [],
  };
  ...
  if (!configuredSheet && metrics.length === 0) {
    ...
    if (best && best.metrics.length > 0) {
      ...
      cacheContext = getCacheContext(workbookPath, sheetName);
    }
  }
